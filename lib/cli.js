...
commander_1.program
    ...
    const config = (0, tools_js_1.mergeConfig)(options);
    const noapi = (0, noapi_js_1.createNoApi)(config);
    const result = yield noapi.fetchDataSource();
    const swFilePath = path_1.default.resolve(config.swFile || 'noapi-swagger-doc.json');
    yield (0, tools_js_1.writeToFile)(swFilePath, JSON.stringify(result, null, 2));
    console.log('文档更新成功！');
}));
