    ...
    generateApiCode(_a, receiveHandler_1) {
        return __awaiter(this, arguments, void 0, function* ({ url, method: onlyMethod, filePath, funcName, }, receiveHandler) {
            ...
            methodKeys.forEach((method) => {
                ...
                if (api.parameters) {
                    ...
                    if (queryParams.length > 0) {
                        ...
                        const interfaceCode = (0, transform_js_1.transformTypeInterfaceCode)(queryParams, queryName);
                        const fileName = `${queryName}.ts`;
                        const filePath = path_1.default.join(this.config.definition.outDir, 'query', fileName);
                        receiveHandler({
                ...
                const apiContext = {
                    ...
                    url,
                    pathParams,
                    inType: queryName ? (0, tools_js_1.defPrefix)(queryName) : undefined,
          ...
          export function ${name}(${paramStr}) {
            return request<${resStr}>({ url: ${urlStr},${inType ? ' data,' : ''} method: '${method}' });
          }
        `;
                    if (comment) {
                        apiFuncStr =
